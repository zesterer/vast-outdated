...
void main()
{
	...
	COLOUR = vec3(0.9, 0.9, 1.0);//texture2D(RENDER_TEXTURE, pos).rgb;
	
	//Just an effect
	COLOUR += getPerlin(vec4(pos, TIME / 500.0, 0.0), 2.0, 3.0, 1.0) * 0.3 - 0.3;

	//Faded corners
	COLOUR *= mix(vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0), min(1, 1.5 - length(UV + 0.3)));
